...
A lane ribbon SHALL be displayed whenever a build tool is selected and SHALL disappear when the
tool is deselected or a placed structure is selected instead. It SHALL be present during the
build phase with no enemies on the board, and during an active wave.
...
The ribbon SHALL draw one lane per **currently active** spawn, following the inbound routing to
the treasury, plus exactly one lane from the treasury following the returning routing to whichever
active spawn it reaches. Each lane SHALL be the tile sequence a follower of that routing would
actually visit, so a lane never crosses a blocked tile and never clips a blocked corner.

#### Scenario: One lane per active spawn plus one out

- **WHEN** the ribbon is shown on a level with two active spawns
- **THEN** three lanes are drawn: one from each spawn to the treasury, and one from the treasury
...
- **THEN** no lane is drawn from that spawn, and one appears once it activates

#### Scenario: The return lane may not go where the enemy came from

- **WHEN** two spawns are active and the treasury's cheapest route out leads to the second spawn
- **THEN** the single return lane runs to that second spawn, not to the first
