...
Pause and fast-forward as properties of the render loop rather than of the simulation, so a player
can stop a live wave to read the board and act on it, and can skip the stretches that have already
been decided — without any of it reaching simulation state, the state hash, or the outcome.
