...
- [ ] 1.1 In `src/render/enemies.ts`, add an exported pure `hpFraction(hp, maxHp)` returning the
      remaining fraction clamped to `[0, 1]` (`0` for a non-positive max) (design D5).
- [ ] 1.2 Extend `EnemyRenderer`'s constructor to take the per-type max hp alongside the type
      keys, and pass `data.enemyTypes.map((t) => t.hp)` from `src/app/game.ts` (design D1).
- [ ] 1.3 Unit-test `hpFraction` in a new `tests/enemies.test.ts` (or the nearest existing
      render-side test file): full, partial, zero/negative hp, non-positive max.

## 2. The bar

- [ ] 2.1 In `src/render/enemies.ts`, add module-level bar constants (base width, height, height
...
- [ ] 2.2 Add a per-enemy `bars` map of `THREE.Group`s: a red track quad at full width and a
...
- [ ] 2.3 In `sync()`, for each enemy with `hp < maxHp[typeId]`: lazily create the group, set
      ...
      (design D3).
- [ ] 2.4 Sweep the bar group in the existing dead-id cleanup loop alongside mesh, shadow, and
      icons.

## 3. Docs

- [ ] 3.1 ARCHITECTURE.md §8: add the health bar to the render-only motion/indicator list and
...
- [ ] 4.1 `npm run typecheck` and `npm test` pass (no sim changes — replay goldens must be
      untouched).
- [ ] 4.2 Playwright exploratory pass (per project apply guidance): no bars on a fresh wave;
