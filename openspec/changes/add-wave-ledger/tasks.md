...
- [ ] 4.1 Lift the leak harness's `powerRun` driver (or its scripted-run core) into `tests/helpers.ts` so `tests/ledger.test.ts` can step the same power-aware scripts tick by tick
- [ ] 4.2 Assert on every tick of both harness scripts: `openingMg + bountiesMg + bonusMg + interestMg − constructionMg − billMg − stolenMg + recoveredMg === treasuryMg`, and `engagedMp + standbyMp + solarWastedMp === solarUsedMp + gridMp + unmetMp` — on the open period; and at each settlement that the closed period satisfies the gold identity against the new period's `openingMg`
- [ ] 4.3 Extend one script so the identity run also covers a removal refund, a connection upgrade and a mid-wave concede, if the existing scripts do not already
