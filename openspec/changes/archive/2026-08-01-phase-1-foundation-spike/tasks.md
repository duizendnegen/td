...
- [x] 2.1 `sim/fixed.ts` — `TILE`/`HALF`/`DIAG` constants, tile↔unit helpers, the single
      `normalize` with `Math.trunc(Math.sqrt(...))`; `tests/fixed.test.ts` (normalisation
      exactness, truncation at negatives, no float leaks)
- [x] 2.2 `sim/rng.ts` — xoshiro128** with `Math.imul` throughout, seedable; known-answer tests
      against published reference vectors (design D-P1-6), plus same-seed-same-sequence test
- [x] 2.3 `sim/types.ts` — sim state and enemy entity types (integer fields only, `prevPos`
      alongside `pos`, absolute-tick timers)
- [x] 2.4 `sim/hash.ts` — FNV-1a canonical walk over ALL sim state (tick, RNG state, treasury,
      every entity field in insertion order), colocated with types; test: single-field change
      changes hash
- [x] 2.5 `sim/commands.ts` — command types, queue, drain sorted by type then issue sequence
- [x] 2.6 `sim/sim.ts` — `Sim` class with the documented tick order (snapshot prevPos → commands →
...
- [x] 3.1 `sim/grid.ts` — tile storage and blocked mask for 30×20
- [x] 3.2 `sim/flowfield.ts` — dual multi-source Dijkstra (inbound from treasury, returning from
...
- [x] 3.3 `tests/flowfield.test.ts` — reachability; costs monotonic toward source; no diagonal
      between corner-to-corner blocked pairs; enclosed tile marked unreachable; same mask → same
      field
- [x] 3.4 `data/schema.ts` — zod schemas per ARCHITECTURE §10 with `waves` allowed empty (spec:
      level-data); float rates converted to integers once at load; load-time spawn→treasury
      reachability check
- [x] 3.5 `tests/level.test.ts` — schema rejects bad spawn refs, unknown enemy types,
      out-of-bounds/blocked treasury or spawns, sealed levels; accepts `waves: []`
- [x] 3.6 Author `level_01.json` instrumented gauntlet (design D-P1-4: S-curve, ≥2
