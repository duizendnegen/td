...
- **Supply merit order** each wave tick: solar first, then the grid — up to capacity and up to what the positive balance can pay. Surplus solar is wasted: not stored, not sold, not carried over. (The slot between solar and grid is where a battery slots in.)
...
- **Solar panels build like walls** — same terrain (dirt only), same validation, same provisional window and committed refund, same removal and move rules — and differ in two ways: the power step reads their output, and they are not foundations — a tower stands on a wall or a socket, never on a panel, so a panel costs you a maze tile a tower could otherwise have used.
